...
Framework:GetJobEvent(function(job)
    currentJob = job.name
    StartGarageBlips()
end)

function StartGarageBlips()
    if not Garage.Garages then return end
...
function OpenGarage(info)
    ...
    for i = 1, #vehicles do
        ...
        if info.shareGarage then
            ...
            shared = data.garage == info.garage
        end
        print(data.garage, info.garage)

        if shared then
            vehiclesFound = true
            if data.stored == 0 or data.stored == 2 then
                data.garage = locale('VehEnLaCalle')
                color = '#FF8787'
            end
            ...
            onSelect = function()
                ...
                for posis = 1, #data.spawn do
                    ...
                    if SpawnClearArea(pos, 2.0) then
                        TriggerServerEvent('mono_garage:RetirarVehiculo', data.plate, data.garage, pos, v.w, data.model,
                            data.intocar)
                        SpawPos = true
                        break
                    end
