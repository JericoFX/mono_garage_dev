...
function Framework.Functions.GetJob(source)
    local player = Framework.Core.GetPlayerFromId(source).getJob().name
    return player
end
...
function Framework.Functions.RemoveMoney(source, money, where)
    ...
    if player.getAccount(where) >= money then
        player.removeAccountMoney(where, (money.price or money))
    else
        return false
    end
end
